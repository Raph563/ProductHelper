...
STATE_FILENAME="${STATE_FILENAME:-grocy-product-helper-state.json}"
ADDON_TARGET_FILENAME="${ADDON_TARGET_FILENAME:-custom_js_product_helper.html}"
...
compose_custom_js() {
	...
	TMP_FILE="$(mktemp "${TMPDIR:-/tmp}/grocy-product-helper-sidecar-compose.XXXXXX")"
	trap 'rm -f "$TMP_FILE"' EXIT
	printf '<!-- managed by docker-sidecar entrypoint (Grocy_Product_Helper) -->\n' > "$TMP_FILE"
	ADDED=0
	OLD_IFS="$IFS"
	IFS=','
